...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Not Known longuri or shortname".encode())
            return
        ...
        if CheckURI(longuri):
            ...
            self.end_headers()
            self.wfile.write("Redirected after '{}'".format(shortname).encode())
        else:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("'{}' could not be fetched".format(longuri).encode())
